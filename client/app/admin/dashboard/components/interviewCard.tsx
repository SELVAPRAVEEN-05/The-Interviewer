...
export const InterviewCard = ({ interview, isOngoing = false, classnames }: InterviewCardProps) => (
  <div className={`bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow ${classnames} `} >
    <div className="flex items-center justify-between mb-3">
      ...
      {isOngoing && 'status' in interview && (
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ${interview.status === 'In Progress' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
            }`}
        >
          {interview.status}
        </span>
