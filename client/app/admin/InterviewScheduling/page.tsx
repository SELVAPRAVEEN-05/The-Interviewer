...
const InterviewScheduling: React.FC = () => {
  ...
  const filteredInterviews = interviews.filter((interview) => {
    ...
    if (dateFilter === "today") {
      ...
      matchesDate =
        new Date(interview.date) <= weekFromNow &&
        new Date(interview.date) >= new Date();
    } else if (dateFilter === "month") {
      const monthFromNow = new Date();
      monthFromNow.setMonth(monthFromNow.getMonth() + 1);
      matchesDate =
        new Date(interview.date) <= monthFromNow &&
        new Date(interview.date) >= new Date();
    }
  ...
  const handleCreateInterview = () => {
    const newInterview: Interview = {
      ...
      candidateName:
        mockCandidates.find((c) => c.id === formData.candidateId)?.name || "",
      candidateEmail:
        mockCandidates.find((c) => c.id === formData.candidateId)?.email || "",
      candidatePosition:
        mockCandidates.find((c) => c.id === formData.candidateId)?.position ||
        "",
      interviewerId: formData.interviewerId,
      interviewerName:
        mockInterviewers.find((i) => i.id === formData.interviewerId)?.name ||
        "",
      interviewerRole:
        mockInterviewers.find((i) => i.id === formData.interviewerId)?.role ||
        "",
      date: formData.date,
      time: formData.time,
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Schedule, manage and track all interviews
            </p>
          </div>
          <button
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500">
                Upcoming interviews organized by date
              </p>
            </div>

            <div className="space-y-6">
              {/* Group interviews by date */}
              {Array.from(new Set(filteredInterviews.map((i) => i.date)))
                .sort()
                .map((date) => (
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const selectedType = e.target
                            .value as Interview["type"];
                          const typeData = interviewTypes.find(
                        ...
                        {(() => {
                          ...
                          return (
                            <div className="text-sm text-blue-700">
                              <p>
                                Timezone: {interviewer.availability.timezone}
                              </p>
                              <p>
                                Hours: {interviewer.availability.workingHours}
                              </p>
                              <p>
                                Preferred Days:{" "}
                                {interviewer.availability.preferredDays.join(
                                  ", "
                                )}
                              </p>
                            </div>
              ...
              <div className="p-6">
                ...
                <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div className="font-medium text-gray-900">
                    {selectedInterview.candidateName} with{" "}
